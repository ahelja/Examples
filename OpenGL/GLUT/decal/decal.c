...
void copyStencilToColor(GLenum whichColorBuffer)
{
    int		x, y;
    GLint	previousColorBuffer;
	GLint	previousPackAlign, previousUnpackAlign;

	glGetIntegerv(GL_PACK_ALIGNMENT, &previousPackAlign);
	glGetIntegerv(GL_UNPACK_ALIGNMENT, &previousUnpackAlign);
	...
	glPixelStorei(GL_PACK_ALIGNMENT, previousPackAlign);
	glPixelStorei(GL_UNPACK_ALIGNMENT, previousUnpackAlign);
}
